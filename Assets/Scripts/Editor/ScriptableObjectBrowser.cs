...
class ScriptableObjectBrowser : EditorWindow
{
	...
	public static GUIStyle ButtonStyle
	{
		get
		{
			...
			{
				_buttonStyle = EditorStyles.whiteLargeLabel;
	...
	private void OnGUI()
	{
		if (_showingTypes)
		{
			GUILayout.Label("Scriptable Object Types", EditorStyles.boldLabel);
		...
		else
		{
			...
			GUIStyle internalStyle = EditorStyles.miniButton;
			Texture icon = (Texture)AssetDatabase.LoadAssetAtPath("Assets/Scripts/Editor/EditorIcons/Button-Refresh-icon.png", typeof(Texture2D));
			Texture icon2 = (Texture)AssetDatabase.LoadAssetAtPath("Assets/Scripts/Editor/EditorIcons/back-icon.png", typeof(Texture2D));

			GUIContent iconRefresh = new GUIContent("Refresh", icon,"Refresh!");
			GUIContent iconBack = new GUIContent("Back to Types", icon2, "Back to list");
			
			//Testing styles
			if (GUILayout.Button(iconRefresh, internalStyle))
			{
				GetAssets();
			}

			if (GUILayout.Button(iconBack,internalStyle))
			{
				GetTypes();
				_showingTypes = true;
			}
